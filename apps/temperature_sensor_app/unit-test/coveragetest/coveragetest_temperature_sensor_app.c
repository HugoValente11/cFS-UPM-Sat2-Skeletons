...
#include "temperature_sensor_app_coveragetest_common.h"
#include "ut_temperature_sensor_app.h"
...
void Test_TEMPERATURE_SENSOR_APP_Main(void)
{
    CFE_SB_MsgId_t MsgId = CFE_SB_INVALID_MSG_ID;

    // Nominal
    TEMPERATURE_SENSOR_APP_Main();
    ...
    TEMPERATURE_SENSOR_APP_Main();
    UtAssert_True(TEMPERATURE_SENSOR_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR,
                  "TEMPERATURE_SENSOR_APP_Data.RunStatus (%lu) == CFE_ES_RunStatus_APP_ERROR",
                  (unsigned long)TEMPERATURE_SENSOR_APP_Data.RunStatus);
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_CheckEvent_Setup(&EventTest, TEMPERATURE_SENSOR_PIPE_ERR_EID, "TEMPERATURE_SENSOR APP: SB Pipe Read Error, App Will Exit");
        ...
        UtAssert_True(TEMPERATURE_SENSOR_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "TEMPERATURE_SENSOR_APP_Data.RunStatus (%lu) == CFE_ES_RunStatus_APP_ERROR", (unsigned long)TEMPERATURE_SENSOR_APP_Data.RunStatus);
...
void Test_TEMPERATURE_SENSOR_APP_Init(void)
{
    // Nominal and check event
    UT_CheckEvent_t EventTest;
    UT_CheckEvent_Setup(&EventTest, TEMPERATURE_SENSOR_STARTUP_INF_EID, "TEMPERATURE_SENSOR App Initialized.%s");
    UT_TEST_FUNCTION_RC(TEMPERATURE_SENSOR_APP_Init(), CFE_SUCCESS);
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_TEST_FUNCTION_RC(TEMPERATURE_SENSOR_APP_Init(), failureRegisterEventCodes[i]);
    ...
    UT_TEST_FUNCTION_RC(TEMPERATURE_SENSOR_APP_Init(), CFE_MSG_BAD_ARGUMENT);
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_TEST_FUNCTION_RC(TEMPERATURE_SENSOR_APP_Init(), failureCreatePipeCodes[i]);
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_TEST_FUNCTION_RC(TEMPERATURE_SENSOR_APP_Init(), failureSubscribeCodes[i]);
...
void Test_TEMPERATURE_SENSOR_FindAppId(void)
{
    ...
    TEMPERATURE_SENSOR_APP_Data.AppId = CFE_ES_APPID_UNDEFINED;
    UT_TEST_FUNCTION_RC(TEMPERATURE_SENSOR_FindAppId(), CFE_ES_ERR_NAME_NOT_FOUND);
    ...
    UtAssert_True(TEMPERATURE_SENSOR_APP_Data.AppId == CFE_ES_APPID_UNDEFINED, "TEMPERATURE_SENSOR_APP_Data.AppId should remain unchanged");
    ...
    UT_TEST_FUNCTION_RC(TEMPERATURE_SENSOR_FindAppId(), CFE_SUCCESS);
    UtAssert_True(TEMPERATURE_SENSOR_APP_Data.AppId == ExpectedAppId, "App ID correctly identified and updated in app data structure.");
    ...
    UT_TEST_FUNCTION_RC(TEMPERATURE_SENSOR_FindAppId(), CFE_ES_BAD_ARGUMENT);
    UtAssert_True(TEMPERATURE_SENSOR_APP_Data.AppId == CFE_ES_APPID_UNDEFINED, "App ID remains unchanged due to CFE_ES_GetAppIDByName returning an error.");
    ...
    UT_TEST_FUNCTION_RC(TEMPERATURE_SENSOR_FindAppId(), CFE_ES_ERR_NAME_NOT_FOUND);
    UtAssert_True(TEMPERATURE_SENSOR_APP_Data.AppId == CFE_ES_APPID_UNDEFINED, "App ID remains unchanged as the app name was not found.");
    ...
    strcpy(CFE_ES_Global.AppTable[CFE_PLATFORM_ES_MAX_APPLICATIONS - 1].AppName, "TEMPERATURE_SENSOR_APP");
    UT_SetDataBuffer(UT_KEY(CFE_ES_GetAppIDByName), &ExpectedAppId, sizeof(ExpectedAppId), false);
    UT_SetDeferredRetcode(UT_KEY(CFE_ES_GetAppIDByName), 1, CFE_SUCCESS);
    TEMPERATURE_SENSOR_FindAppId();
    UtAssert_True(TEMPERATURE_SENSOR_APP_Data.AppId == ExpectedAppId, "App ID successfully found even when the match is the last app in the table.");
    ...
    TEMPERATURE_SENSOR_APP_Data.AppId = CFE_ES_APPID_UNDEFINED;
    TEMPERATURE_SENSOR_FindAppId();
    UtAssert_True(TEMPERATURE_SENSOR_APP_Data.AppId == CFE_ES_APPID_UNDEFINED, "App ID remains undefined when AppTable is empty, as expected.");
...
void Test_TEMPERATURE_SENSOR_Send_Sensor_data_Init(void)
{
    // Nominal case: Ensuring message initialization sets message ID and size correctly without error
    UT_ResetState(0);
    memset(&TEMPERATURE_SENSOR_APP_Data, 0, sizeof(TEMPERATURE_SENSOR_APP_Data));
    UT_TEST_FUNCTION_RC(TEMPERATURE_SENSOR_Send_Sensor_data_Init(), CFE_SUCCESS);
    ...
    UtAssert_True(TEMPERATURE_SENSOR_APP_Data.MsgId_Sensor_data != 0, "Message ID should be set during initialization");
    UtAssert_True(TEMPERATURE_SENSOR_APP_Data.Size_Sensor_data != 0, "Message size should be defined during initialization");
    ...
    for (int i = 0; i < sizeof(failureCodes)/sizeof(failureCodes[0]); ++i) {
        ...
        UT_TEST_FUNCTION_RC(TEMPERATURE_SENSOR_Send_Sensor_data_Init(), failureCodes[i]);
        UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Syslog should be written for message initialization failure");
    }
}

void Test_TEMPERATURE_SENSOR_User_Send_Sensor_data(void)
{
    ...
    UT_TEST_FUNCTION_RC(TEMPERATURE_SENSOR_User_Send_Sensor_data(), CFE_SUCCESS);
    UtAssert_True(TEMPERATURE_SENSOR_APP_Data.RunStatus == CFE_ES_RunStatus_APP_RUN, "Message successfully encoded and sent.");

    // Test case: Failure during the encoding process
    UT_SetDeferredRetcode(UT_KEY(asn1SccMyInteger_Encode), 1, false);
    UT_TEST_FUNCTION_RC(TEMPERATURE_SENSOR_User_Send_Sensor_data(), CFE_SB_BAD_ARGUMENT);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Encoding failure leads to an error log entry.");
    UtAssert_True(TEMPERATURE_SENSOR_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "Encoding failure changes the application run status to error.");
    ...
    UT_TEST_FUNCTION_RC(TEMPERATURE_SENSOR_User_Send_Sensor_data(), CFE_PSP_ERROR);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 2, "Memory copy failure leads to an error log entry.");
    UtAssert_True(TEMPERATURE_SENSOR_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "Memory copy failure changes the application run status to error.");
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_TEST_FUNCTION_RC(TEMPERATURE_SENSOR_User_Send_Sensor_data(), failureCodes[i]);
        UtAssert_True(TEMPERATURE_SENSOR_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "Transmission failure changes the application run status to error.");
...
void Test_TEMPERATURE_SENSOR_ProcessCommandPacket(void)
{
    /*
     * Test Case For:
     * void TEMPERATURE_SENSOR_ProcessCommandPacket
     */
    /* a buffer large enough for any command message */
    union
    {
        CFE_SB_Buffer_t      SBBuf;
        TEMPERATURE_SENSOR_NoopCmd_t Noop;
    } TestMsg;
    ...
    UT_CheckEvent_Setup(&EventTest, TEMPERATURE_SENSOR_INVALID_MSGID_ERR_EID, "TEMPERATURE_SENSOR: invalid command packet,MID = 0x%x");

    /*
     * The CFE_MSG_GetMsgId() stub uses a data buffer to hold the
     * message ID values to return.
     */
    TestMsgId = CFE_SB_ValueToMsgId(TEMPERATURE_SENSOR_CMD_MID);
    FcnCode   = TEMPERATURE_SENSOR_NOOP_CC;
    MsgSize   = sizeof(TestMsg.Noop);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &MsgSize, sizeof(MsgSize), false);
    TEMPERATURE_SENSOR_ProcessCommandPacket(&TestMsg.SBBuf);

    TestMsgId = CFE_SB_ValueToMsgId(TEMPERATURE_SENSOR_SEND_HK_MID);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    TEMPERATURE_SENSOR_ProcessCommandPacket(&TestMsg.SBBuf);
     ...
     */
    UtAssert_True(EventTest.MatchCount == 1, "TEMPERATURE_SENSOR_COMMAND_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_TEMPERATURE_SENSOR_ProcessGroundCommand(void)
{
    /*
     * Test Setup for Function : TEMPERATURE_SENSOR_ProcessGroundCommand
     */
    CFE_MSG_FcnCode_t FcnCode;
    size_t Size;
    ...
    union
    {
        CFE_SB_Buffer_t SBBuf;
        TEMPERATURE_SENSOR_NoopCmd_t Noop;
        TEMPERATURE_SENSOR_ResetCountersCmd_t Reset;
        TEMPERATURE_SENSOR_ProcessCmd_t Process;
    } TestMsg;
    ...
    TEMPERATURE_SENSOR_ProcessGroundCommand(&TestMsg.SBBuf);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 0, "CFE_ES_WriteToSysLog() should not be called");
    ...
    // Correct length
    FcnCode = TEMPERATURE_SENSOR_RESET_COUNTERS_CC;
...
void Test_TEMPERATURE_SENSOR_ReportHousekeeping(void)
{
    /*
     * Test Case For:
     * void TEMPERATURE_SENSOR_ReportHousekeeping( const CFE_SB_CmdHdr_t *Msg )
     */
    CFE_MSG_Message_t *MsgSend;
    CFE_MSG_Message_t *MsgTimestamp;
    CFE_SB_MsgId_t     MsgId = CFE_SB_ValueToMsgId(TEMPERATURE_SENSOR_SEND_HK_MID);

    /* Set message id to return so TEMPERATURE_SENSOR_Housekeeping will be called */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &MsgId, sizeof(MsgId), false);
    ...
    TEMPERATURE_SENSOR_ProcessCommandPacket((CFE_SB_Buffer_t *)NULL);

    /* Confirm message sent*/
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_SB_TransmitMsg)) == 1, "CFE_SB_TransmitMsg() called once");
    UtAssert_True(MsgSend == &TEMPERATURE_SENSOR_APP_Data.HkTlm.TlmHeader.Msg, "CFE_SB_TransmitMsg() address matches expected");

    /* Confirm timestamp msg address */
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_SB_TimeStampMsg)) == 1, "CFE_SB_TimeStampMsg() called once");
    UtAssert_True(MsgTimestamp == &TEMPERATURE_SENSOR_APP_Data.HkTlm.TlmHeader.Msg,
                  "CFE_SB_TimeStampMsg() address matches expected");

}

void Test_TEMPERATURE_SENSOR_Noop(void)
{
    /*
     * Test Case For:
     * void TEMPERATURE_SENSOR_NoopCmd( const TEMPERATURE_SENSOR_Noop_t *Msg )
     */
    TEMPERATURE_SENSOR_NoopCmd_t TestMsg;
    UT_CheckEvent_t      EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    /* test dispatch of NOOP */
    UT_CheckEvent_Setup(&EventTest, TEMPERATURE_SENSOR_COMMANDNOP_INF_EID, NULL);

    UT_TEST_FUNCTION_RC(TEMPERATURE_SENSOR_Noop(&TestMsg), CFE_SUCCESS);

    /*
     * Confirm that the event was generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "TEMPERATURE_SENSOR_COMMANDNOP_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_TEMPERATURE_SENSOR_ResetCounters(void)
{
    /*
     * Test Case For:
     * void TEMPERATURE_SENSOR_ResetCounters( const TEMPERATURE_SENSOR_ResetCounters_t *Msg )
     */
    TEMPERATURE_SENSOR_ResetCountersCmd_t TestMsg;
    UT_CheckEvent_t               EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CheckEvent_Setup(&EventTest, TEMPERATURE_SENSOR_COMMANDRST_INF_EID, "TEMPERATURE_SENSOR: RESET command");

    UT_TEST_FUNCTION_RC(TEMPERATURE_SENSOR_ResetCounters(&TestMsg), CFE_SUCCESS);

    /*
     * Confirm that the event was generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "TEMPERATURE_SENSOR_COMMANDRST_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}


void Test_TEMPERATURE_SENSOR_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool TEMPERATURE_SENSOR_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
     ...
     */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &size, sizeof(size), false);
    UT_CheckEvent_Setup(&EventTest, TEMPERATURE_SENSOR_LEN_ERR_EID,
                        "Invalid Msg length: ID = 0x%X,  CC = %u, Len = %u, Expected = %u");

    TEMPERATURE_SENSOR_VerifyCmdLength(NULL, size);

    /*
     * Confirm that the event was NOT generated
     */
    UtAssert_True(EventTest.MatchCount == 0, "TEMPERATURE_SENSOR_LEN_ERR_EID NOT generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &msgid, sizeof(msgid), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    TEMPERATURE_SENSOR_VerifyCmdLength(NULL, size + 1);

    /*
     * Confirm that the event WAS generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "TEMPERATURE_SENSOR_LEN_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}
...
{
    ADD_TEST(TEMPERATURE_SENSOR_APP_Init);
    ADD_TEST(TEMPERATURE_SENSOR_APP_Main);
    ADD_TEST(TEMPERATURE_SENSOR_FindAppId);
    ADD_TEST(TEMPERATURE_SENSOR_Send_Sensor_data_Init);
    ADD_TEST(TEMPERATURE_SENSOR_User_Send_Sensor_data);

    
    
    ADD_TEST(TEMPERATURE_SENSOR_ProcessCommandPacket);
    ADD_TEST(TEMPERATURE_SENSOR_ProcessGroundCommand);
    ADD_TEST(TEMPERATURE_SENSOR_ReportHousekeeping);
    ADD_TEST(TEMPERATURE_SENSOR_Noop);
    ADD_TEST(TEMPERATURE_SENSOR_ResetCounters);
    ADD_TEST(TEMPERATURE_SENSOR_VerifyCmdLength);
