...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *  * *  * * * * **/
/* PRESSURE_SENSOR_AppMain() -- Application entry point and main process loop         */
/*                                                                            */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *  * *  * * * * **/
void PRESSURE_SENSOR_APP_Main(void)
{
    ...
    */
    status = PRESSURE_SENSOR_Init();
    if (status != CFE_SUCCESS)
    {
        PRESSURE_SENSOR_Data.RunStatus = CFE_ES_RunStatus_APP_ERROR;
    }
    ...
    // Main app loop
    while (CFE_ES_RunLoop(&PRESSURE_SENSOR_Data.RunStatus) == true)
    {
        // Call user loop
    ...
    status = CFE_SB_DeletePipe (PRESSURE_SENSOR_Data.CommandPipe);
    ...
    */
    ...
    PRESSURE_SENSOR_Data.RunStatus = CFE_ES_RunStatus_APP_EXIT;

    CFE_ES_ExitApp(PRESSURE_SENSOR_Data.RunStatus);

} /* End of PRESSURE_SENSOR_APP_Main() */

/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *  */
/*                                                                            */
/* PRESSURE_SENSOR_Init() --  initialization                                       */
/*                                                                            */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * **/
int32 PRESSURE_SENSOR_Init(void)
{
    int32 status;

    PRESSURE_SENSOR_Data.RunStatus = CFE_ES_RunStatus_APP_RUN;

    /*
    ** Initialize app command execution counters
    */
    PRESSURE_SENSOR_Data.CmdCounter = 0;
    PRESSURE_SENSOR_Data.ErrCounter = 0;

    /*
    ** Initialize app configuration data
    */
    PRESSURE_SENSOR_Data.PipeDepth = PRESSURE_SENSOR_PIPE_DEPTH;

    strncpy(PRESSURE_SENSOR_Data.PipeName, "PRESSURE_SENSOR_CMD_PIPE", sizeof(PRESSURE_SENSOR_Data.PipeName));
    PRESSURE_SENSOR_Data.PipeName[sizeof(PRESSURE_SENSOR_Data.PipeName) - 1] = 0;

    /*
    ** Initialize event filter table...
    */
    PRESSURE_SENSOR_Data.EventFilters[0].EventID = PRESSURE_SENSOR_STARTUP_INF_EID;
    PRESSURE_SENSOR_Data.EventFilters[0].Mask    = 0x0000;
    PRESSURE_SENSOR_Data.EventFilters[1].EventID = PRESSURE_SENSOR_COMMAND_ERR_EID;
    PRESSURE_SENSOR_Data.EventFilters[1].Mask    = 0x0000;
    PRESSURE_SENSOR_Data.EventFilters[2].EventID = PRESSURE_SENSOR_COMMANDNOP_INF_EID;
    PRESSURE_SENSOR_Data.EventFilters[2].Mask    = 0x0000;
    PRESSURE_SENSOR_Data.EventFilters[3].EventID = PRESSURE_SENSOR_COMMANDRST_INF_EID;
    PRESSURE_SENSOR_Data.EventFilters[3].Mask    = 0x0000;
    PRESSURE_SENSOR_Data.EventFilters[4].EventID = PRESSURE_SENSOR_INVALID_MSGID_ERR_EID;
    PRESSURE_SENSOR_Data.EventFilters[4].Mask    = 0x0000;
    PRESSURE_SENSOR_Data.EventFilters[5].EventID = PRESSURE_SENSOR_LEN_ERR_EID;
    PRESSURE_SENSOR_Data.EventFilters[5].Mask    = 0x0000;
    PRESSURE_SENSOR_Data.EventFilters[6].EventID = PRESSURE_SENSOR_PIPE_ERR_EID;
    PRESSURE_SENSOR_Data.EventFilters[6].Mask    = 0x0000;
    

    /*
    ** Register the events
    */
    status = CFE_EVS_Register(PRESSURE_SENSOR_Data.EventFilters, PRESSURE_SENSOR_EVENT_COUNTS, CFE_EVS_EventFilter_BINARY);
    if (status != CFE_SUCCESS)
    {
    ...
    */
    CFE_MSG_Init(&PRESSURE_SENSOR_Data.HkTlm.TlmHeader.Msg, CFE_SB_ValueToMsgId(PRESSURE_SENSOR_HK_TLM_MID),
                 sizeof(PRESSURE_SENSOR_Data.HkTlm));

    /*
    ** Create Software Bus message pipe.
    */
    status = CFE_SB_CreatePipe(&PRESSURE_SENSOR_Data.CommandPipe, PRESSURE_SENSOR_Data.PipeDepth, PRESSURE_SENSOR_Data.PipeName);
    if (status != CFE_SUCCESS)
    {
    ...
    */
    status = CFE_SB_Subscribe(CFE_SB_ValueToMsgId(PRESSURE_SENSOR_SEND_HK_MID), PRESSURE_SENSOR_Data.CommandPipe);
    if (status != CFE_SUCCESS)
    {
    ...
    */
    status = CFE_SB_Subscribe(CFE_SB_ValueToMsgId(PRESSURE_SENSOR_CMD_MID), PRESSURE_SENSOR_Data.CommandPipe);
    if (status != CFE_SUCCESS)
    {
    ...
    */
    ...
    strcpy(PRESSURE_SENSOR_Data.PipeName, "PRESSURE_SENSOR_COM_PIPE");
    CFE_ES_WriteToSysLog("[PRESSURE_SENSOR] COM_PIPE Name = %s", PRESSURE_SENSOR_Data.PipeName);

    status = CFE_SB_CreatePipe(&PRESSURE_SENSOR_Data.CommandPipe, PRESSURE_SENSOR_Data.PipeDepth, PRESSURE_SENSOR_Data.PipeName);
    if (status != CFE_SUCCESS)
    {
        CFE_ES_WriteToSysLog("[PRESSURE_SENSOR] Error creating communication pipe,210, PRESSURE_SENSOR_app.c, RC = 0x%08lX\n",
                             (unsigned long)status);
    }
    CFE_ES_WriteToSysLog("[PRESSURE_SENSOR] Pipe creation successful");
...
int32 PRESSURE_SENSOR_ReportHousekeeping(const CFE_MSG_CommandHeader_t *Msg)
{
    ...
    */
    PRESSURE_SENSOR_Data.HkTlm.Payload.CommandErrorCounter = PRESSURE_SENSOR_Data.ErrCounter;
    PRESSURE_SENSOR_Data.HkTlm.Payload.CommandCounter      = PRESSURE_SENSOR_Data.CmdCounter;

    /*
    ** Send housekeeping telemetry packet...
    */
    CFE_SB_TimeStampMsg(&PRESSURE_SENSOR_Data.HkTlm.TlmHeader.Msg);
    CFE_SB_TransmitMsg(&PRESSURE_SENSOR_Data.HkTlm.TlmHeader.Msg, true);
...
int32 PRESSURE_SENSOR_Noop(const PRESSURE_SENSOR_NoopCmd_t *Msg)
{
    ...
    PRESSURE_SENSOR_Data.CmdCounter++;
...
bool PRESSURE_SENSOR_VerifyCmdLength(CFE_MSG_Message_t *MsgPtr, size_t ExpectedLength)
{
    ...
    */
    if (ExpectedLength != ActualLength)
    {
        ...
        PRESSURE_SENSOR_Data.ErrCounter++;
    }
...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
/*                                                                 */
/* PRESSURE_SENSOR_Send_Sensor_data -- Send message         */
/*                                                                 */
/*                                                                 */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
void PRESSURE_SENSOR_Send_Sensor_data() {
    // Initialize message Sensor_data
    PRESSURE_SENSOR_Data.MsgId_Sensor_data = PRESSURE_SENSOR_APP_COMMUNICATION_MID_10;
    PRESSURE_SENSOR_Data.Size_Sensor_data = asn1SccMyInteger_REQUIRED_BITS_FOR_ENCODING;

    status = CFE_MSG_Init(&PRESSURE_SENSOR_Data.MsgPtr_Sensor_data, PRESSURE_SENSOR_Data.MsgId_Sensor_data, PRESSURE_SENSOR_Data.Size_Sensor_data);
    ...
    // Encode
    if (!asn1SccMyInteger_Encode(&PRESSURE_SENSOR_Data.OUT_pSensorData, &bitstream, &errorCode, TRUE))
    {
        CFE_ES_WriteToSysLog("[PRESSURE_SENSOR] Encode failed. Error code %d.\n", errorCode);
    }


    status = CFE_PSP_MemCpy(CFE_SB_GetUserData(&PRESSURE_SENSOR_Data.MsgPtr_Sensor_data), &perBuffer, sizeof(perBuffer));
    ...
    status = CFE_SB_TransmitMsg(&PRESSURE_SENSOR_Data.MsgPtr_Sensor_data, false);

    if (status != CFE_SUCCESS) {
        PRESSURE_SENSOR_Data.RunStatus = CFE_ES_RunStatus_APP_ERROR;
        CFE_ES_WriteToSysLog("[PRESSURE_SENSOR] Error sending the message, RC = %lu\n", (unsigned long) status);
    } else {
        // CFE_ES_WriteToSysLog("[PRESSURE_SENSOR] Message sent successfully");
    }
...
/*                                                                 */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
void PRESSURE_SENSOR_FindAppId(void)
{
    /*
    ...
    */
    for (int i = 0; i < CFE_PLATFORM_ES_MAX_APPLICATIONS; i++ )
    {
        ...
        {
            CFE_ES_GetAppIDByName(&PRESSURE_SENSOR_Data.AppId, CFE_ES_Global.AppTable[i].AppName);
        }
