...
#include "manager_app_coveragetest_common.h"
#include "ut_manager_app.h"
...
void Test_MANAGER_APP_Main(void)
{
    ...
    MANAGER_APP_Main();
    UtAssert_True(MANAGER_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR,
                  "MANAGER_APP_Data.RunStatus (%lu) == CFE_ES_RunStatus_APP_ERROR",
                  (unsigned long)MANAGER_APP_Data.RunStatus);
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_CheckEvent_Setup(&EventTest, MANAGER_PIPE_ERR_EID, "MANAGER APP: SB Pipe Read Error, App Will Exit");
        ...
        UtAssert_True(MANAGER_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "MANAGER_APP_Data.RunStatus (%lu) == CFE_ES_RunStatus_APP_ERROR", (unsigned long)MANAGER_APP_Data.RunStatus);
...
void Test_MANAGER_APP_Init(void)
{
    // Nominal and check event
    UT_CheckEvent_t EventTest;
    UT_CheckEvent_Setup(&EventTest, MANAGER_STARTUP_INF_EID, "MANAGER App Initialized.%s");
    UT_TEST_FUNCTION_RC(MANAGER_APP_Init(), CFE_SUCCESS);
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_TEST_FUNCTION_RC(MANAGER_APP_Init(), failureRegisterEventCodes[i]);
    ...
    UT_TEST_FUNCTION_RC(MANAGER_APP_Init(), CFE_MSG_BAD_ARGUMENT);
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_TEST_FUNCTION_RC(MANAGER_APP_Init(), failureCreatePipeCodes[i]);
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_TEST_FUNCTION_RC(MANAGER_APP_Init(), failureSubscribeCodes[i]);
...
void Test_MANAGER_FindAppId(void)
{
    ...
    MANAGER_APP_Data.AppId = CFE_ES_APPID_UNDEFINED;
    UT_TEST_FUNCTION_RC(MANAGER_FindAppId(), CFE_ES_ERR_NAME_NOT_FOUND);
    ...
    UtAssert_True(MANAGER_APP_Data.AppId == CFE_ES_APPID_UNDEFINED, "MANAGER_APP_Data.AppId should remain unchanged");

    // Success on finding app
    // Initialize AppTable with matching names
    ExpectedAppId = 42;
    strcpy(CFE_ES_Global.AppTable[0].AppName, "MANAGER_APP");
    ...
    UT_TEST_FUNCTION_RC(MANAGER_FindAppId(), CFE_SUCCESS);
    UtAssert_True(MANAGER_APP_Data.AppId == ExpectedAppId, "App ID correctly identified and updated in app data structure.");
    ...
    UT_TEST_FUNCTION_RC(MANAGER_FindAppId(), CFE_ES_BAD_ARGUMENT);
    UtAssert_True(MANAGER_APP_Data.AppId == CFE_ES_APPID_UNDEFINED, "App ID remains unchanged due to CFE_ES_GetAppIDByName returning an error.");
    ...
    UT_TEST_FUNCTION_RC(MANAGER_FindAppId(), CFE_ES_ERR_NAME_NOT_FOUND);
    UtAssert_True(MANAGER_APP_Data.AppId == CFE_ES_APPID_UNDEFINED, "App ID remains unchanged as the app name was not found.");
    ...
    strcpy(CFE_ES_Global.AppTable[CFE_PLATFORM_ES_MAX_APPLICATIONS - 1].AppName, "MANAGER_APP");
    UT_SetDataBuffer(UT_KEY(CFE_ES_GetAppIDByName), &ExpectedAppId, sizeof(ExpectedAppId), false);
    UT_SetDeferredRetcode(UT_KEY(CFE_ES_GetAppIDByName), 1, CFE_SUCCESS);
    MANAGER_FindAppId();
    UtAssert_True(MANAGER_APP_Data.AppId == ExpectedAppId, "App ID successfully found even when the match is the last app in the table.");
    ...
    MANAGER_APP_Data.AppId = CFE_ES_APPID_UNDEFINED;
    MANAGER_FindAppId();
    UtAssert_True(MANAGER_APP_Data.AppId == CFE_ES_APPID_UNDEFINED, "App ID remains undefined when AppTable is empty, as expected.");
...
void Test_MANAGER_Send_Current_Mode_Init(void)
{
    // Nominal case: Ensuring message initialization sets message ID and size correctly without error
    UT_ResetState(0);
    memset(&MANAGER_APP_Data, 0, sizeof(MANAGER_APP_Data));
    UT_TEST_FUNCTION_RC(MANAGER_Send_Current_Mode_Init(), CFE_SUCCESS);
    ...
    UtAssert_True(MANAGER_APP_Data.MsgId_Current_Mode != 0, "Message ID should be set during initialization");
    UtAssert_True(MANAGER_APP_Data.Size_Current_Mode != 0, "Message size should be defined during initialization");
...
void Test_MANAGER_User_Send_Current_Mode(void)
{
    ...
    UT_TEST_FUNCTION_RC(MANAGER_User_Send_Current_Mode(), CFE_SUCCESS);
    UtAssert_True(MANAGER_APP_Data.RunStatus == CFE_ES_RunStatus_APP_RUN, "Message successfully encoded and sent.");

    // Test case: Failure during the encoding process
    UT_SetDeferredRetcode(UT_KEY(asn1SccMyInteger_Encode), 1, false);
    UT_TEST_FUNCTION_RC(MANAGER_User_Send_Current_Mode(), CFE_SB_BAD_ARGUMENT);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Encoding failure leads to an error log entry.");
    UtAssert_True(MANAGER_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "Encoding failure changes the application run status to error.");
    ...
    UT_TEST_FUNCTION_RC(MANAGER_User_Send_Current_Mode(), CFE_PSP_ERROR);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 2, "Memory copy failure leads to an error log entry.");
    UtAssert_True(MANAGER_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "Memory copy failure changes the application run status to error.");
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_TEST_FUNCTION_RC(MANAGER_User_Send_Current_Mode(), failureCodes[i]);
        UtAssert_True(MANAGER_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "Transmission failure changes the application run status to error.");
        UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Transmission failure leads to an error log entry.");
    }
}
void Test_MANAGER_Receive_Process_TC_Init(void)
{
    // Nominal case: Subscription success
    UT_TEST_FUNCTION_RC(MANAGER_Receive_Process_TC_Init(), CFE_SUCCESS);
    ...
    for (int i = 0; i < sizeof(failureCodes)/sizeof(failureCodes[0]); ++i) {
        ...
        UT_TEST_FUNCTION_RC(MANAGER_Receive_Process_TC_Init(), failureCodes[i]);
        UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Syslog should be written for subscription failure");
    }
}

void Test_MANAGER_User_Receive_Process_TC(void)
{
    ...
    MANAGER_APP_Data.Buffer = &mockBuffer;

    // Fail on decoding
    UT_SetDeferredRetcode(UT_KEY(asn1SccMyInteger_Decode), 1, false);
    UT_TEST_FUNCTION_RC(MANAGER_User_Receive_Process_TC(), CFE_SB_BAD_ARGUMENT);
    ...
    UT_TEST_FUNCTION_RC(MANAGER_User_Receive_Process_TC(), CFE_SUCCESS);
...
void Test_MANAGER_User_TM_Error_Event(void)
{
    // Test case: Successful event sending
    UT_CheckEvent_t EventTest;
    UT_CheckEvent_Setup(&EventTest,
    MANAGER_TM_ERROR_EID, "[MANAGER-ERROR] Error");
    UT_TEST_FUNCTION_RC(MANAGER_User_TM_Error_Event(), CFE_SUCCESS);
    UtAssert_True(EventTest.MatchCount == 1, "Successfully generated the TM_Error event with correct information.");
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_TEST_FUNCTION_RC(MANAGER_User_TM_Error_Event(), failureCodes[i]);
...
void Test_MANAGER_User_TM_Mode_Event(void)
{
    // Test case: Successful event sending
    UT_CheckEvent_t EventTest;
    UT_CheckEvent_Setup(&EventTest,
    MANAGER_TM_MODE_EID, "[MANAGER-INFORMATION] Current mode");
    UT_TEST_FUNCTION_RC(MANAGER_User_TM_Mode_Event(), CFE_SUCCESS);
    UtAssert_True(EventTest.MatchCount == 1, "Successfully generated the TM_Mode event with correct information.");
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_EVS_SendEvent)) == 1, "CFE_EVS_SendEvent was correctly called once.");
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 0, "No SysLog entries should be generated for successful event sending.");
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_ResetState(UT_KEY(CFE_ES_WriteToSysLog));
        UT_SetDeferredRetcode(UT_KEY(CFE_EVS_SendEvent), 1, failureCodes[i]);
        UT_TEST_FUNCTION_RC(MANAGER_User_TM_Mode_Event(), failureCodes[i]);
        UtAssert_True(UT_GetStubCount(UT_KEY(CFE_EVS_SendEvent)) == 1, "Attempted to send an event despite failure scenario.");
        UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Generated a SysLog entry due to the failure to send event.");
    }
}

void Test_MANAGER_ProcessCommandPacket(void)
{
    /*
     * Test Case For:
     * void MANAGER_ProcessCommandPacket
     */
    /* a buffer large enough for any command message */
    union
    {
        CFE_SB_Buffer_t      SBBuf;
        MANAGER_NoopCmd_t Noop;
    } TestMsg;
    ...
    UT_CheckEvent_Setup(&EventTest, MANAGER_INVALID_MSGID_ERR_EID, "MANAGER: invalid command packet,MID = 0x%x");

    /*
     * The CFE_MSG_GetMsgId() stub uses a data buffer to hold the
     * message ID values to return.
     */
    TestMsgId = CFE_SB_ValueToMsgId(MANAGER_CMD_MID);
    FcnCode   = MANAGER_NOOP_CC;
    MsgSize   = sizeof(TestMsg.Noop);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &MsgSize, sizeof(MsgSize), false);
    MANAGER_ProcessCommandPacket(&TestMsg.SBBuf);

    TestMsgId = CFE_SB_ValueToMsgId(MANAGER_SEND_HK_MID);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    MANAGER_ProcessCommandPacket(&TestMsg.SBBuf);

    // Process_TC message id
    TestMsgId = CFE_SB_ValueToMsgId(0x017);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    MANAGER_ProcessCommandPacket(&TestMsg.SBBuf);
     ...
     */
    UtAssert_True(EventTest.MatchCount == 1, "MANAGER_COMMAND_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_MANAGER_ProcessGroundCommand(void)
{
    /*
     * Test Setup for Function : MANAGER_ProcessGroundCommand
     */
    CFE_MSG_FcnCode_t FcnCode;
    size_t Size;
    ...
    union
    {
        CFE_SB_Buffer_t SBBuf;
        MANAGER_NoopCmd_t Noop;
        MANAGER_ResetCountersCmd_t Reset;
        MANAGER_ProcessCmd_t Process;
    } TestMsg;
    ...
    MANAGER_ProcessGroundCommand(&TestMsg.SBBuf);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 0, "CFE_ES_WriteToSysLog() should not be called");
    ...
    // Correct length
    FcnCode = MANAGER_RESET_COUNTERS_CC;
...
void Test_MANAGER_ReportHousekeeping(void)
{
    /*
     * Test Case For:
     * void MANAGER_ReportHousekeeping( const CFE_SB_CmdHdr_t *Msg )
     */
    CFE_MSG_Message_t *MsgSend;
    CFE_MSG_Message_t *MsgTimestamp;
    CFE_SB_MsgId_t     MsgId = CFE_SB_ValueToMsgId(MANAGER_SEND_HK_MID);

    /* Set message id to return so MANAGER_Housekeeping will be called */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &MsgId, sizeof(MsgId), false);
    ...
    MANAGER_ProcessCommandPacket((CFE_SB_Buffer_t *)NULL);

    /* Confirm message sent*/
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_SB_TransmitMsg)) == 1, "CFE_SB_TransmitMsg() called once");
    UtAssert_True(MsgSend == &MANAGER_APP_Data.HkTlm.TlmHeader.Msg, "CFE_SB_TransmitMsg() address matches expected");

    /* Confirm timestamp msg address */
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_SB_TimeStampMsg)) == 1, "CFE_SB_TimeStampMsg() called once");
    UtAssert_True(MsgTimestamp == &MANAGER_APP_Data.HkTlm.TlmHeader.Msg,
                  "CFE_SB_TimeStampMsg() address matches expected");

}

void Test_MANAGER_Noop(void)
{
    /*
     * Test Case For:
     * void MANAGER_NoopCmd( const MANAGER_Noop_t *Msg )
     */
    MANAGER_NoopCmd_t TestMsg;
    UT_CheckEvent_t      EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    /* test dispatch of NOOP */
    UT_CheckEvent_Setup(&EventTest, MANAGER_COMMANDNOP_INF_EID, NULL);

    UT_TEST_FUNCTION_RC(MANAGER_Noop(&TestMsg), CFE_SUCCESS);

    /*
     * Confirm that the event was generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "MANAGER_COMMANDNOP_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_MANAGER_ResetCounters(void)
{
    /*
     * Test Case For:
     * void MANAGER_ResetCounters( const MANAGER_ResetCounters_t *Msg )
     */
    MANAGER_ResetCountersCmd_t TestMsg;
    UT_CheckEvent_t               EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CheckEvent_Setup(&EventTest, MANAGER_COMMANDRST_INF_EID, "MANAGER: RESET command");

    UT_TEST_FUNCTION_RC(MANAGER_ResetCounters(&TestMsg), CFE_SUCCESS);

    /*
     * Confirm that the event was generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "MANAGER_COMMANDRST_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}


void Test_MANAGER_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool MANAGER_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
     ...
     */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &size, sizeof(size), false);
    UT_CheckEvent_Setup(&EventTest, MANAGER_LEN_ERR_EID,
                        "Invalid Msg length: ID = 0x%X,  CC = %u, Len = %u, Expected = %u");

    MANAGER_VerifyCmdLength(NULL, size);

    /*
     * Confirm that the event was NOT generated
     */
    UtAssert_True(EventTest.MatchCount == 0, "MANAGER_LEN_ERR_EID NOT generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &msgid, sizeof(msgid), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    MANAGER_VerifyCmdLength(NULL, size + 1);

    /*
     * Confirm that the event WAS generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "MANAGER_LEN_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}
...
{
    ADD_TEST(MANAGER_APP_Init);
    ADD_TEST(MANAGER_APP_Main);
    ADD_TEST(MANAGER_FindAppId);
    ADD_TEST(MANAGER_Send_Current_Mode_Init);
    ADD_TEST(MANAGER_User_Send_Current_Mode);
ADD_TEST(MANAGER_Receive_Process_TC_Init);
    ADD_TEST(MANAGER_User_Receive_Process_TC);
    
    ADD_TEST(MANAGER_User_TM_Error_Event);


ADD_TEST(MANAGER_User_TM_Mode_Event);
    ADD_TEST(MANAGER_ProcessCommandPacket);
    ADD_TEST(MANAGER_ProcessGroundCommand);
    ADD_TEST(MANAGER_ReportHousekeeping);
    ADD_TEST(MANAGER_Noop);
    ADD_TEST(MANAGER_ResetCounters);
    ADD_TEST(MANAGER_VerifyCmdLength);
