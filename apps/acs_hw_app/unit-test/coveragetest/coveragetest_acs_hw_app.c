...
#include "acs_hw_app_coveragetest_common.h"
#include "ut_acs_hw_app.h"
...
void Test_ACS_HW_APP_Main(void)
{
    ...
    ACS_HW_APP_Main();
    UtAssert_True(ACS_HW_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR,
                  "ACS_HW_APP_Data.RunStatus (%lu) == CFE_ES_RunStatus_APP_ERROR",
                  (unsigned long)ACS_HW_APP_Data.RunStatus);
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_CheckEvent_Setup(&EventTest, ACS_HW_PIPE_ERR_EID, "ACS_HW APP: SB Pipe Read Error, App Will Exit");
        ...
        UtAssert_True(ACS_HW_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "ACS_HW_APP_Data.RunStatus (%lu) == CFE_ES_RunStatus_APP_ERROR", (unsigned long)ACS_HW_APP_Data.RunStatus);
...
void Test_ACS_HW_APP_Init(void)
{
    // Nominal and check event
    UT_CheckEvent_t EventTest;
    UT_CheckEvent_Setup(&EventTest, ACS_HW_STARTUP_INF_EID, "ACS_HW App Initialized.%s");
    UT_TEST_FUNCTION_RC(ACS_HW_APP_Init(), CFE_SUCCESS);
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_TEST_FUNCTION_RC(ACS_HW_APP_Init(), failureRegisterEventCodes[i]);
    ...
    UT_TEST_FUNCTION_RC(ACS_HW_APP_Init(), CFE_MSG_BAD_ARGUMENT);
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_TEST_FUNCTION_RC(ACS_HW_APP_Init(), failureCreatePipeCodes[i]);
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_TEST_FUNCTION_RC(ACS_HW_APP_Init(), failureSubscribeCodes[i]);
...
void Test_ACS_HW_FindAppId(void)
{
    ...
    ACS_HW_APP_Data.AppId = CFE_ES_APPID_UNDEFINED;
    UT_TEST_FUNCTION_RC(ACS_HW_FindAppId(), CFE_ES_ERR_NAME_NOT_FOUND);
    ...
    UtAssert_True(ACS_HW_APP_Data.AppId == CFE_ES_APPID_UNDEFINED, "ACS_HW_APP_Data.AppId should remain unchanged");

    // Success on finding app
    // Initialize AppTable with matching names
    ExpectedAppId = 42;
    strcpy(CFE_ES_Global.AppTable[0].AppName, "ACS_HW_APP");
    ...
    UT_TEST_FUNCTION_RC(ACS_HW_FindAppId(), CFE_SUCCESS);
    UtAssert_True(ACS_HW_APP_Data.AppId == ExpectedAppId, "App ID correctly identified and updated in app data structure.");
    ...
    UT_TEST_FUNCTION_RC(ACS_HW_FindAppId(), CFE_ES_BAD_ARGUMENT);
    UtAssert_True(ACS_HW_APP_Data.AppId == CFE_ES_APPID_UNDEFINED, "App ID remains unchanged due to CFE_ES_GetAppIDByName returning an error.");
    ...
    UT_TEST_FUNCTION_RC(ACS_HW_FindAppId(), CFE_ES_ERR_NAME_NOT_FOUND);
    UtAssert_True(ACS_HW_APP_Data.AppId == CFE_ES_APPID_UNDEFINED, "App ID remains unchanged as the app name was not found.");
    ...
    strcpy(CFE_ES_Global.AppTable[CFE_PLATFORM_ES_MAX_APPLICATIONS - 1].AppName, "ACS_HW_APP");
    UT_SetDataBuffer(UT_KEY(CFE_ES_GetAppIDByName), &ExpectedAppId, sizeof(ExpectedAppId), false);
    UT_SetDeferredRetcode(UT_KEY(CFE_ES_GetAppIDByName), 1, CFE_SUCCESS);
    ACS_HW_FindAppId();
    UtAssert_True(ACS_HW_APP_Data.AppId == ExpectedAppId, "App ID successfully found even when the match is the last app in the table.");
    ...
    ACS_HW_APP_Data.AppId = CFE_ES_APPID_UNDEFINED;
    ACS_HW_FindAppId();
    UtAssert_True(ACS_HW_APP_Data.AppId == CFE_ES_APPID_UNDEFINED, "App ID remains undefined when AppTable is empty, as expected.");
...
void Test_ACS_HW_Send_MGM_Data_Init(void)
{
    // Nominal case: Ensuring message initialization sets message ID and size correctly without error
    UT_ResetState(0);
    memset(&ACS_HW_APP_Data, 0, sizeof(ACS_HW_APP_Data));
    UT_TEST_FUNCTION_RC(ACS_HW_Send_MGM_Data_Init(), CFE_SUCCESS);
    ...
    UtAssert_True(ACS_HW_APP_Data.MsgId_MGM_Data != 0, "Message ID should be set during initialization");
    UtAssert_True(ACS_HW_APP_Data.Size_MGM_Data != 0, "Message size should be defined during initialization");
...
void Test_ACS_HW_User_Send_MGM_Data(void)
{
    ...
    UT_TEST_FUNCTION_RC(ACS_HW_User_Send_MGM_Data(), CFE_SUCCESS);
    UtAssert_True(ACS_HW_APP_Data.RunStatus == CFE_ES_RunStatus_APP_RUN, "Message successfully encoded and sent.");

    // Test case: Failure during the encoding process
    UT_SetDeferredRetcode(UT_KEY(asn1SccMyInteger_Encode), 1, false);
    UT_TEST_FUNCTION_RC(ACS_HW_User_Send_MGM_Data(), CFE_SB_BAD_ARGUMENT);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Encoding failure leads to an error log entry.");
    UtAssert_True(ACS_HW_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "Encoding failure changes the application run status to error.");
    ...
    UT_TEST_FUNCTION_RC(ACS_HW_User_Send_MGM_Data(), CFE_PSP_ERROR);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 2, "Memory copy failure leads to an error log entry.");
    UtAssert_True(ACS_HW_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "Memory copy failure changes the application run status to error.");
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_TEST_FUNCTION_RC(ACS_HW_User_Send_MGM_Data(), failureCodes[i]);
        UtAssert_True(ACS_HW_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "Transmission failure changes the application run status to error.");
        UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Transmission failure leads to an error log entry.");
    }
}
void Test_ACS_HW_Receive_MGT_Data_Init(void)
{
    // Nominal case: Subscription success
    UT_TEST_FUNCTION_RC(ACS_HW_Receive_MGT_Data_Init(), CFE_SUCCESS);
...
void Test_ACS_HW_User_Receive_MGT_Data(void)
{
    ...
    ACS_HW_APP_Data.Buffer = &mockBuffer;

    // Fail on decoding
    UT_SetDeferredRetcode(UT_KEY(asn1SccMyInteger_Decode), 1, false);
    UT_TEST_FUNCTION_RC(ACS_HW_User_Receive_MGT_Data(), CFE_SB_BAD_ARGUMENT);
    ...
    UT_TEST_FUNCTION_RC(ACS_HW_User_Receive_MGT_Data(), CFE_SUCCESS);
...
void Test_ACS_HW_ProcessCommandPacket(void)
{
    /*
     * Test Case For:
     * void ACS_HW_ProcessCommandPacket
     */
    /* a buffer large enough for any command message */
    union
    {
        CFE_SB_Buffer_t      SBBuf;
        ACS_HW_NoopCmd_t Noop;
    } TestMsg;
    ...
    UT_CheckEvent_Setup(&EventTest, ACS_HW_INVALID_MSGID_ERR_EID, "ACS_HW: invalid command packet,MID = 0x%x");

    /*
     * The CFE_MSG_GetMsgId() stub uses a data buffer to hold the
     * message ID values to return.
     */
    TestMsgId = CFE_SB_ValueToMsgId(ACS_HW_CMD_MID);
    FcnCode   = ACS_HW_NOOP_CC;
    MsgSize   = sizeof(TestMsg.Noop);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &MsgSize, sizeof(MsgSize), false);
    ACS_HW_ProcessCommandPacket(&TestMsg.SBBuf);

    TestMsgId = CFE_SB_ValueToMsgId(ACS_HW_SEND_HK_MID);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    ACS_HW_ProcessCommandPacket(&TestMsg.SBBuf);

    // MGT_Data message id
    TestMsgId = CFE_SB_ValueToMsgId(0x015);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    ACS_HW_ProcessCommandPacket(&TestMsg.SBBuf);
     ...
     */
    UtAssert_True(EventTest.MatchCount == 1, "ACS_HW_COMMAND_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_ACS_HW_ProcessGroundCommand(void)
{
    /*
     * Test Setup for Function : ACS_HW_ProcessGroundCommand
     */
    CFE_MSG_FcnCode_t FcnCode;
    size_t Size;
    ...
    union
    {
        CFE_SB_Buffer_t SBBuf;
        ACS_HW_NoopCmd_t Noop;
        ACS_HW_ResetCountersCmd_t Reset;
        ACS_HW_ProcessCmd_t Process;
    } TestMsg;
    ...
    ACS_HW_ProcessGroundCommand(&TestMsg.SBBuf);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 0, "CFE_ES_WriteToSysLog() should not be called");
    ...
    // Correct length
    FcnCode = ACS_HW_RESET_COUNTERS_CC;
...
void Test_ACS_HW_ReportHousekeeping(void)
{
    /*
     * Test Case For:
     * void ACS_HW_ReportHousekeeping( const CFE_SB_CmdHdr_t *Msg )
     */
    CFE_MSG_Message_t *MsgSend;
    CFE_MSG_Message_t *MsgTimestamp;
    CFE_SB_MsgId_t     MsgId = CFE_SB_ValueToMsgId(ACS_HW_SEND_HK_MID);

    /* Set message id to return so ACS_HW_Housekeeping will be called */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &MsgId, sizeof(MsgId), false);
    ...
    ACS_HW_ProcessCommandPacket((CFE_SB_Buffer_t *)NULL);

    /* Confirm message sent*/
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_SB_TransmitMsg)) == 1, "CFE_SB_TransmitMsg() called once");
    UtAssert_True(MsgSend == &ACS_HW_APP_Data.HkTlm.TlmHeader.Msg, "CFE_SB_TransmitMsg() address matches expected");

    /* Confirm timestamp msg address */
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_SB_TimeStampMsg)) == 1, "CFE_SB_TimeStampMsg() called once");
    UtAssert_True(MsgTimestamp == &ACS_HW_APP_Data.HkTlm.TlmHeader.Msg,
                  "CFE_SB_TimeStampMsg() address matches expected");

}

void Test_ACS_HW_Noop(void)
{
    /*
     * Test Case For:
     * void ACS_HW_NoopCmd( const ACS_HW_Noop_t *Msg )
     */
    ACS_HW_NoopCmd_t TestMsg;
    UT_CheckEvent_t      EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    /* test dispatch of NOOP */
    UT_CheckEvent_Setup(&EventTest, ACS_HW_COMMANDNOP_INF_EID, NULL);

    UT_TEST_FUNCTION_RC(ACS_HW_Noop(&TestMsg), CFE_SUCCESS);

    /*
     * Confirm that the event was generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "ACS_HW_COMMANDNOP_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_ACS_HW_ResetCounters(void)
{
    /*
     * Test Case For:
     * void ACS_HW_ResetCounters( const ACS_HW_ResetCounters_t *Msg )
     */
    ACS_HW_ResetCountersCmd_t TestMsg;
    UT_CheckEvent_t               EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CheckEvent_Setup(&EventTest, ACS_HW_COMMANDRST_INF_EID, "ACS_HW: RESET command");

    UT_TEST_FUNCTION_RC(ACS_HW_ResetCounters(&TestMsg), CFE_SUCCESS);

    /*
     * Confirm that the event was generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "ACS_HW_COMMANDRST_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}


void Test_ACS_HW_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool ACS_HW_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
     ...
     */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &size, sizeof(size), false);
    UT_CheckEvent_Setup(&EventTest, ACS_HW_LEN_ERR_EID,
                        "Invalid Msg length: ID = 0x%X,  CC = %u, Len = %u, Expected = %u");

    ACS_HW_VerifyCmdLength(NULL, size);

    /*
     * Confirm that the event was NOT generated
     */
    UtAssert_True(EventTest.MatchCount == 0, "ACS_HW_LEN_ERR_EID NOT generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &msgid, sizeof(msgid), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    ACS_HW_VerifyCmdLength(NULL, size + 1);

    /*
     * Confirm that the event WAS generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "ACS_HW_LEN_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}
...
{
    ADD_TEST(ACS_HW_APP_Init);
    ADD_TEST(ACS_HW_APP_Main);
    ADD_TEST(ACS_HW_FindAppId);
    ADD_TEST(ACS_HW_Send_MGM_Data_Init);
    ADD_TEST(ACS_HW_User_Send_MGM_Data);
ADD_TEST(ACS_HW_Receive_MGT_Data_Init);
    ADD_TEST(ACS_HW_User_Receive_MGT_Data);
    
    
    ADD_TEST(ACS_HW_ProcessCommandPacket);
    ADD_TEST(ACS_HW_ProcessGroundCommand);
    ADD_TEST(ACS_HW_ReportHousekeeping);
    ADD_TEST(ACS_HW_Noop);
    ADD_TEST(ACS_HW_ResetCounters);
    ADD_TEST(ACS_HW_VerifyCmdLength);
