...
#include "measurer_and_actuator_app_coveragetest_common.h"
#include "ut_measurer_and_actuator_app.h"
...
void Test_MEASURER_AND_ACTUATOR_APP_Main(void)
{
    CFE_SB_MsgId_t MsgId = CFE_SB_INVALID_MSG_ID;

    // Nominal
    MEASURER_AND_ACTUATOR_APP_Main();
    ...
    MEASURER_AND_ACTUATOR_APP_Main();
    UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR,
                  "MEASURER_AND_ACTUATOR_APP_Data.RunStatus (%lu) == CFE_ES_RunStatus_APP_ERROR",
                  (unsigned long)MEASURER_AND_ACTUATOR_APP_Data.RunStatus);
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_CheckEvent_Setup(&EventTest, MEASURER_AND_ACTUATOR_PIPE_ERR_EID, "MEASURER_AND_ACTUATOR APP: SB Pipe Read Error, App Will Exit");
        ...
        UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "MEASURER_AND_ACTUATOR_APP_Data.RunStatus (%lu) == CFE_ES_RunStatus_APP_ERROR", (unsigned long)MEASURER_AND_ACTUATOR_APP_Data.RunStatus);
...
void Test_MEASURER_AND_ACTUATOR_APP_Init(void)
{
    // Nominal and check event
    UT_CheckEvent_t EventTest;
    UT_CheckEvent_Setup(&EventTest, MEASURER_AND_ACTUATOR_STARTUP_INF_EID, "MEASURER_AND_ACTUATOR App Initialized.%s");
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_APP_Init(), CFE_SUCCESS);
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_APP_Init(), failureRegisterEventCodes[i]);
    ...
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_APP_Init(), CFE_MSG_BAD_ARGUMENT);
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_APP_Init(), failureCreatePipeCodes[i]);
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_APP_Init(), failureSubscribeCodes[i]);
...
void Test_MEASURER_AND_ACTUATOR_FindAppId(void)
{
    ...
    MEASURER_AND_ACTUATOR_APP_Data.AppId = CFE_ES_APPID_UNDEFINED;
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_FindAppId(), CFE_ES_ERR_NAME_NOT_FOUND);
    ...
    UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.AppId == CFE_ES_APPID_UNDEFINED, "MEASURER_AND_ACTUATOR_APP_Data.AppId should remain unchanged");

    // Success on finding app
    // Initialize AppTable with matching names
    ExpectedAppId = 42;
    strcpy(CFE_ES_Global.AppTable[0].AppName, "MEASURER_AND_ACTUATOR_APP");
    ...
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_FindAppId(), CFE_SUCCESS);
    UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.AppId == ExpectedAppId, "App ID correctly identified and updated in app data structure.");
    ...
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_FindAppId(), CFE_ES_BAD_ARGUMENT);
    UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.AppId == CFE_ES_APPID_UNDEFINED, "App ID remains unchanged due to CFE_ES_GetAppIDByName returning an error.");
    ...
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_FindAppId(), CFE_ES_ERR_NAME_NOT_FOUND);
    UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.AppId == CFE_ES_APPID_UNDEFINED, "App ID remains unchanged as the app name was not found.");
    ...
    strcpy(CFE_ES_Global.AppTable[CFE_PLATFORM_ES_MAX_APPLICATIONS - 1].AppName, "MEASURER_AND_ACTUATOR_APP");
    UT_SetDataBuffer(UT_KEY(CFE_ES_GetAppIDByName), &ExpectedAppId, sizeof(ExpectedAppId), false);
    UT_SetDeferredRetcode(UT_KEY(CFE_ES_GetAppIDByName), 1, CFE_SUCCESS);
    MEASURER_AND_ACTUATOR_FindAppId();
    UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.AppId == ExpectedAppId, "App ID successfully found even when the match is the last app in the table.");
    ...
    MEASURER_AND_ACTUATOR_APP_Data.AppId = CFE_ES_APPID_UNDEFINED;
    MEASURER_AND_ACTUATOR_FindAppId();
    UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.AppId == CFE_ES_APPID_UNDEFINED, "App ID remains undefined when AppTable is empty, as expected.");
...
void Test_MEASURER_AND_ACTUATOR_Send_MGT_Data_Init(void)
{
    // Nominal case: Ensuring message initialization sets message ID and size correctly without error
    UT_ResetState(0);
    memset(&MEASURER_AND_ACTUATOR_APP_Data, 0, sizeof(MEASURER_AND_ACTUATOR_APP_Data));
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_Send_MGT_Data_Init(), CFE_SUCCESS);
    ...
    UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.MsgId_MGT_Data != 0, "Message ID should be set during initialization");
    UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.Size_MGT_Data != 0, "Message size should be defined during initialization");
    ...
    for (int i = 0; i < sizeof(failureCodes)/sizeof(failureCodes[0]); ++i) {
        ...
        UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_Send_MGT_Data_Init(), failureCodes[i]);
        UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Syslog should be written for message initialization failure");
    }
}

void Test_MEASURER_AND_ACTUATOR_User_Send_MGT_Data(void)
{
    ...
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_User_Send_MGT_Data(), CFE_SUCCESS);
    UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.RunStatus == CFE_ES_RunStatus_APP_RUN, "Message successfully encoded and sent.");

    // Test case: Failure during the encoding process
    UT_SetDeferredRetcode(UT_KEY(asn1SccMyInteger_Encode), 1, false);
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_User_Send_MGT_Data(), CFE_SB_BAD_ARGUMENT);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Encoding failure leads to an error log entry.");
    UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "Encoding failure changes the application run status to error.");
    ...
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_User_Send_MGT_Data(), CFE_PSP_ERROR);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 2, "Memory copy failure leads to an error log entry.");
    UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "Memory copy failure changes the application run status to error.");
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_User_Send_MGT_Data(), failureCodes[i]);
        UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "Transmission failure changes the application run status to error.");
        UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Transmission failure leads to an error log entry.");
    }
}


void Test_MEASURER_AND_ACTUATOR_Send_Step_In_Init(void)
{
    // Nominal case: Ensuring message initialization sets message ID and size correctly without error
    UT_ResetState(0);
    memset(&MEASURER_AND_ACTUATOR_APP_Data, 0, sizeof(MEASURER_AND_ACTUATOR_APP_Data));
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_Send_Step_In_Init(), CFE_SUCCESS);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 0, "No syslog should be written for successful message initialization");
    UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.MsgId_Step_In != 0, "Message ID should be set during initialization");
    UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.Size_Step_In != 0, "Message size should be defined during initialization");
    ...
    for (int i = 0; i < sizeof(failureCodes)/sizeof(failureCodes[0]); ++i) {
        UT_ResetState(0);
        UT_SetDeferredRetcode(UT_KEY(CFE_MSG_Init), 1, failureCodes[i]);
        UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_Send_Step_In_Init(), failureCodes[i]);
        UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Syslog should be written for message initialization failure");
    }
}

void Test_MEASURER_AND_ACTUATOR_User_Send_Step_In(void)
{
    // Test case: Successful message encoding and transmission
    MEASURER_AND_ACTUATOR_APP_Data.RunStatus = CFE_ES_RunStatus_APP_RUN;
    UT_SetDeferredRetcode(UT_KEY(asn1SccMyInteger_Encode), 1, true);
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_User_Send_Step_In(), CFE_SUCCESS);
    UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.RunStatus == CFE_ES_RunStatus_APP_RUN, "Message successfully encoded and sent.");

    // Test case: Failure during the encoding process
    UT_SetDeferredRetcode(UT_KEY(asn1SccMyInteger_Encode), 1, false);
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_User_Send_Step_In(), CFE_SB_BAD_ARGUMENT);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Encoding failure leads to an error log entry.");
    UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "Encoding failure changes the application run status to error.");
    ...
    UT_SetDeferredRetcode(UT_KEY(asn1SccMyInteger_Encode), 1, true); // Encoding succeeds
    UT_SetDeferredRetcode(UT_KEY(CFE_PSP_MemCpy), 1, CFE_PSP_ERROR); // Memory copy fails
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_User_Send_Step_In(), CFE_PSP_ERROR);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 2, "Memory copy failure leads to an error log entry.");
    UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "Memory copy failure changes the application run status to error.");
    ...
    for (int i = 0; i < numFailures; i++)
    {
        ...
        UT_SetDeferredRetcode(UT_KEY(asn1SccMyInteger_Encode), 1, true); // Assume encoding succeeds
        UT_SetDeferredRetcode(UT_KEY(CFE_SB_TransmitMsg), 1, failureCodes[i]);
        UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_User_Send_Step_In(), failureCodes[i]);
        UtAssert_True(MEASURER_AND_ACTUATOR_APP_Data.RunStatus == CFE_ES_RunStatus_APP_ERROR, "Transmission failure changes the application run status to error.");
        UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Transmission failure leads to an error log entry.");
    }
}
void Test_MEASURER_AND_ACTUATOR_Receive_MGM_Data_Init(void)
{
    // Nominal case: Subscription success
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_Receive_MGM_Data_Init(), CFE_SUCCESS);
    ...
    for (int i = 0; i < sizeof(failureCodes)/sizeof(failureCodes[0]); ++i) {
        ...
        UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_Receive_MGM_Data_Init(), failureCodes[i]);
        UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Syslog should be written for subscription failure");
    }
}

void Test_MEASURER_AND_ACTUATOR_User_Receive_MGM_Data(void)
{
    ...
    MEASURER_AND_ACTUATOR_APP_Data.Buffer = &mockBuffer;

    // Fail on decoding
    UT_SetDeferredRetcode(UT_KEY(asn1SccMyInteger_Decode), 1, false);
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_User_Receive_MGM_Data(), CFE_SB_BAD_ARGUMENT);
    ...
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_User_Receive_MGM_Data(), CFE_SUCCESS);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Successful reception and decoding of the message.");

}


void Test_MEASURER_AND_ACTUATOR_Receive_Step_Out_Init(void)
{
    // Nominal case: Subscription success
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_Receive_Step_Out_Init(), CFE_SUCCESS);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_SB_Subscribe)) == 1, "CFE_SB_Subscribe should be called once for successful subscription");
    ...
    for (int i = 0; i < sizeof(failureCodes)/sizeof(failureCodes[0]); ++i) {
        UT_ResetState(0);
        UT_SetDeferredRetcode(UT_KEY(CFE_SB_Subscribe), 1, failureCodes[i]);
        UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_Receive_Step_Out_Init(), failureCodes[i]);
        UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Syslog should be written for subscription failure");
    }
}

void Test_MEASURER_AND_ACTUATOR_User_Receive_Step_Out(void)
{
    // Mock buffer and message
    CFE_SB_Buffer_t mockBuffer;
    memset(&mockBuffer, 0, sizeof(CFE_SB_Buffer_t)); // Clear the buffer
    ...
    // Fail on decoding
    UT_SetDeferredRetcode(UT_KEY(asn1SccMyInteger_Decode), 1, false);
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_User_Receive_Step_Out(), CFE_SB_BAD_ARGUMENT);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Decode failure logged correctly.");
    ...
    UT_SetDataBuffer(UT_KEY(CFE_SB_GetUserData), mockData, sizeof(mockData), false);
    UT_SetDeferredRetcode(UT_KEY(asn1SccMyInteger_Decode), 1, true);
    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_User_Receive_Step_Out(), CFE_SUCCESS);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "Successful reception and decoding of the message.");
...
void Test_MEASURER_AND_ACTUATOR_ProcessCommandPacket(void)
{
    /*
     * Test Case For:
     * void MEASURER_AND_ACTUATOR_ProcessCommandPacket
     */
    /* a buffer large enough for any command message */
    union
    {
        CFE_SB_Buffer_t      SBBuf;
        MEASURER_AND_ACTUATOR_NoopCmd_t Noop;
    } TestMsg;
    ...
    UT_CheckEvent_Setup(&EventTest, MEASURER_AND_ACTUATOR_INVALID_MSGID_ERR_EID, "MEASURER_AND_ACTUATOR: invalid command packet,MID = 0x%x");

    /*
     * The CFE_MSG_GetMsgId() stub uses a data buffer to hold the
     * message ID values to return.
     */
    TestMsgId = CFE_SB_ValueToMsgId(MEASURER_AND_ACTUATOR_CMD_MID);
    FcnCode   = MEASURER_AND_ACTUATOR_NOOP_CC;
    MsgSize   = sizeof(TestMsg.Noop);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &MsgSize, sizeof(MsgSize), false);
    MEASURER_AND_ACTUATOR_ProcessCommandPacket(&TestMsg.SBBuf);

    TestMsgId = CFE_SB_ValueToMsgId(MEASURER_AND_ACTUATOR_SEND_HK_MID);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    MEASURER_AND_ACTUATOR_ProcessCommandPacket(&TestMsg.SBBuf);

    // MGM_Data message id
    TestMsgId = CFE_SB_ValueToMsgId(0x013);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    MEASURER_AND_ACTUATOR_ProcessCommandPacket(&TestMsg.SBBuf);


    // Step_Out message id
    TestMsgId = CFE_SB_ValueToMsgId(0x012);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    MEASURER_AND_ACTUATOR_ProcessCommandPacket(&TestMsg.SBBuf);
     ...
     */
    UtAssert_True(EventTest.MatchCount == 1, "MEASURER_AND_ACTUATOR_COMMAND_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_MEASURER_AND_ACTUATOR_ProcessGroundCommand(void)
{
    /*
     * Test Setup for Function : MEASURER_AND_ACTUATOR_ProcessGroundCommand
     */
    CFE_MSG_FcnCode_t FcnCode;
    size_t Size;
    ...
    union
    {
        CFE_SB_Buffer_t SBBuf;
        MEASURER_AND_ACTUATOR_NoopCmd_t Noop;
        MEASURER_AND_ACTUATOR_ResetCountersCmd_t Reset;
        MEASURER_AND_ACTUATOR_ProcessCmd_t Process;
    } TestMsg;
    ...
    MEASURER_AND_ACTUATOR_ProcessGroundCommand(&TestMsg.SBBuf);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 0, "CFE_ES_WriteToSysLog() should not be called");
    ...
    // Correct length
    FcnCode = MEASURER_AND_ACTUATOR_RESET_COUNTERS_CC;
...
void Test_MEASURER_AND_ACTUATOR_ReportHousekeeping(void)
{
    /*
     * Test Case For:
     * void MEASURER_AND_ACTUATOR_ReportHousekeeping( const CFE_SB_CmdHdr_t *Msg )
     */
    CFE_MSG_Message_t *MsgSend;
    CFE_MSG_Message_t *MsgTimestamp;
    CFE_SB_MsgId_t     MsgId = CFE_SB_ValueToMsgId(MEASURER_AND_ACTUATOR_SEND_HK_MID);

    /* Set message id to return so MEASURER_AND_ACTUATOR_Housekeeping will be called */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &MsgId, sizeof(MsgId), false);
    ...
    MEASURER_AND_ACTUATOR_ProcessCommandPacket((CFE_SB_Buffer_t *)NULL);

    /* Confirm message sent*/
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_SB_TransmitMsg)) == 1, "CFE_SB_TransmitMsg() called once");
    UtAssert_True(MsgSend == &MEASURER_AND_ACTUATOR_APP_Data.HkTlm.TlmHeader.Msg, "CFE_SB_TransmitMsg() address matches expected");

    /* Confirm timestamp msg address */
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_SB_TimeStampMsg)) == 1, "CFE_SB_TimeStampMsg() called once");
    UtAssert_True(MsgTimestamp == &MEASURER_AND_ACTUATOR_APP_Data.HkTlm.TlmHeader.Msg,
                  "CFE_SB_TimeStampMsg() address matches expected");

}

void Test_MEASURER_AND_ACTUATOR_Noop(void)
{
    /*
     * Test Case For:
     * void MEASURER_AND_ACTUATOR_NoopCmd( const MEASURER_AND_ACTUATOR_Noop_t *Msg )
     */
    MEASURER_AND_ACTUATOR_NoopCmd_t TestMsg;
    UT_CheckEvent_t      EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    /* test dispatch of NOOP */
    UT_CheckEvent_Setup(&EventTest, MEASURER_AND_ACTUATOR_COMMANDNOP_INF_EID, NULL);

    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_Noop(&TestMsg), CFE_SUCCESS);

    /*
     * Confirm that the event was generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "MEASURER_AND_ACTUATOR_COMMANDNOP_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_MEASURER_AND_ACTUATOR_ResetCounters(void)
{
    /*
     * Test Case For:
     * void MEASURER_AND_ACTUATOR_ResetCounters( const MEASURER_AND_ACTUATOR_ResetCounters_t *Msg )
     */
    MEASURER_AND_ACTUATOR_ResetCountersCmd_t TestMsg;
    UT_CheckEvent_t               EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CheckEvent_Setup(&EventTest, MEASURER_AND_ACTUATOR_COMMANDRST_INF_EID, "MEASURER_AND_ACTUATOR: RESET command");

    UT_TEST_FUNCTION_RC(MEASURER_AND_ACTUATOR_ResetCounters(&TestMsg), CFE_SUCCESS);

    /*
     * Confirm that the event was generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "MEASURER_AND_ACTUATOR_COMMANDRST_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}


void Test_MEASURER_AND_ACTUATOR_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool MEASURER_AND_ACTUATOR_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
     ...
     */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &size, sizeof(size), false);
    UT_CheckEvent_Setup(&EventTest, MEASURER_AND_ACTUATOR_LEN_ERR_EID,
                        "Invalid Msg length: ID = 0x%X,  CC = %u, Len = %u, Expected = %u");

    MEASURER_AND_ACTUATOR_VerifyCmdLength(NULL, size);

    /*
     * Confirm that the event was NOT generated
     */
    UtAssert_True(EventTest.MatchCount == 0, "MEASURER_AND_ACTUATOR_LEN_ERR_EID NOT generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &msgid, sizeof(msgid), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    MEASURER_AND_ACTUATOR_VerifyCmdLength(NULL, size + 1);

    /*
     * Confirm that the event WAS generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "MEASURER_AND_ACTUATOR_LEN_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}
...
{
    ADD_TEST(MEASURER_AND_ACTUATOR_APP_Init);
    ADD_TEST(MEASURER_AND_ACTUATOR_APP_Main);
    ADD_TEST(MEASURER_AND_ACTUATOR_FindAppId);
    ADD_TEST(MEASURER_AND_ACTUATOR_Send_MGT_Data_Init);
    ADD_TEST(MEASURER_AND_ACTUATOR_User_Send_MGT_Data);


    ADD_TEST(MEASURER_AND_ACTUATOR_Send_Step_In_Init);
    ADD_TEST(MEASURER_AND_ACTUATOR_User_Send_Step_In);
ADD_TEST(MEASURER_AND_ACTUATOR_Receive_MGM_Data_Init);
    ADD_TEST(MEASURER_AND_ACTUATOR_User_Receive_MGM_Data);


    ADD_TEST(MEASURER_AND_ACTUATOR_Receive_Step_Out_Init);
    ADD_TEST(MEASURER_AND_ACTUATOR_User_Receive_Step_Out);
    
    
    ADD_TEST(MEASURER_AND_ACTUATOR_ProcessCommandPacket);
    ADD_TEST(MEASURER_AND_ACTUATOR_ProcessGroundCommand);
    ADD_TEST(MEASURER_AND_ACTUATOR_ReportHousekeeping);
    ADD_TEST(MEASURER_AND_ACTUATOR_Noop);
    ADD_TEST(MEASURER_AND_ACTUATOR_ResetCounters);
    ADD_TEST(MEASURER_AND_ACTUATOR_VerifyCmdLength);
